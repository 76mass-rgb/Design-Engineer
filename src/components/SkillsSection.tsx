...
export const SkillsSection: React.FC<SkillsSectionProps> = ({ currentLang }) => {
  return (
    <section id="skills" className="py-24 md:py-32 relative border-t border-[var(--border-color)] bg-[var(--bg-primary)]">
      <div className="w-full px-4 sm:px-8 lg:px-12 xl:px-16 2xl:px-20 relative z-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-16">
          {SKILLS_DATA.map((skill) => (
            <div
              key={skill.id}
              className="relative bg-[var(--glass-bg)] p-7 sm:p-9 rounded-3xl border border-[var(--border-color)] hover:border-[var(--accent-blue)] transition-all duration-300 ease-out hover:scale-[1.05] hover:-translate-y-2 hover:shadow-2xl hover:z-30 hover:bg-[var(--bg-surface)] backdrop-blur-xl group shadow-sm flex flex-col justify-between cursor-pointer"
            >
              <div>
                {/* Icon & ID */}
                <div className="flex items-start justify-between mb-5">
                  <span className="text-3xl p-3.5 bg-[var(--badge-bg)] rounded-2xl border border-[var(--border-color)] group-hover:scale-110 transition-transform inline-block">
                    {skill.icon}
                  </span>
                  <span className="text-sm font-mono text-[var(--accent-blue)] uppercase tracking-wider font-extrabold bg-[var(--badge-bg)] px-3.5 py-1.5 rounded-full border border-[var(--border-color)]">
                    {skill.id.replace('-', ' ').toUpperCase()}
                  </span>
                </div>

                <h3 className="text-xl sm:text-2xl font-black text-[var(--text-primary)] mb-3 group-hover:text-[var(--accent-blue)] transition-colors">
                  {skill.name[currentLang]}
                </h3>

                <p className="text-base sm:text-lg text-[var(--text-secondary)] leading-relaxed transition-colors mb-6 font-normal">
              ...
              {skill.tags && (
                <div className="flex flex-wrap gap-2 pt-4 border-t border-[var(--border-color)]">
                  {skill.tags.map((tag, idx) => (
                    <span
                      key={idx}
                      className="text-xs sm:text-sm font-mono uppercase tracking-wider px-3 py-1 bg-[var(--badge-bg)] text-[var(--text-primary)] rounded-md border border-[var(--border-color)] font-bold"
                    >
                      {tag}
                    </span>
                  ))}
        ...
        <div className="bg-[var(--glass-bg)] border border-[var(--border-color)] rounded-3xl p-6 sm:p-8 backdrop-blur-xl shadow-sm">
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6 pb-6 border-b border-[var(--border-color)]">
            <div>
              <div className="text-sm uppercase tracking-[0.2em] text-[var(--accent-blue)] font-extrabold mb-1 font-gost-mono">
                {UI_TRANSLATIONS.softwareProficiency[currentLang]}
              </div>
              <h4 className="font-gost text-xl sm:text-2xl font-bold uppercase text-[var(--text-primary)] tracking-tight">
                {UI_TRANSLATIONS.cadIsometrics[currentLang]}
              </h4>
            </div>
            <div className="flex items-center gap-2 text-sm sm:text-base text-[var(--text-secondary)] font-gost-mono bg-[var(--bg-surface-2)] px-4.5 py-2.5 rounded-full border border-[var(--border-color)] w-fit font-bold">
              <Compass className="w-4.5 h-4.5 text-[var(--accent-blue)]" />
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3 sm:gap-4">
            {ENGINEERING_TOOLS_DATA.tools.map((tool, index) => {
              ...
              return (
                <div
                  key={index}
                  className="relative p-4 sm:p-5 bg-[var(--bg-surface-2)] border border-[var(--border-color)] rounded-2xl transition-all duration-300 ease-out hover:scale-[1.30] sm:hover:scale-[1.35] hover:-translate-y-4 hover:shadow-2xl hover:shadow-black/30 dark:hover:shadow-black/70 hover:border-[var(--accent-blue)] hover:bg-[var(--bg-surface)] hover:z-50 cursor-pointer group flex flex-col justify-between"
                >
                  <div>
                    <div className="font-gost text-base sm:text-lg font-bold text-[var(--text-primary)] group-hover:text-[var(--accent-blue)] transition-colors">
                      {toolName}
                    </div>
                    <div className="text-sm text-[var(--accent-blue)] mt-1 leading-snug font-bold">
                      {toolCategory}
                    </div>
                  </div>
                  <div className="text-xs sm:text-sm text-[var(--text-secondary)] group-hover:text-[var(--text-primary)] mt-2.5 line-clamp-2 group-hover:line-clamp-none leading-relaxed font-medium transition-colors">
                    {toolDesc}
                  </div>
                </div>
              );
